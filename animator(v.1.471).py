...
def save():
    ...
    for x in lstring[1159:]:
        temp.append(list(filter(None, ansi_shortner(x))))
    tframes.append(temp)  
    v_value = []
    plusser = 0
    for x in range(len(tframes[0])):
        try:
            num_o_times = 0
            xx = x + plusser
            for y in range(xx, len(tframes[0])):
                if not tframes[0][xx] == tframes[0][y]:
                    print(tframes[0][xx], tframes[0][y])                 
                if tframes[0][xx] == tframes[0][y]:
                    num_o_times += 1
                elif tframes[0][y] != ['\n']:
                    break
            ...
            #print(num_o_times)  
            if num_o_times != 0:
                v_value.append([tframes[0][x], num_o_times])  
        except:
            break    

    file_content.append(v_value)       
